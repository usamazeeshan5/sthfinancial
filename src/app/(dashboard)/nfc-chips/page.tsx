...
import { Search, Plus, Link2, Unlink, Layers, Download, MoreHorizontal, Eye, Trash2, Power } from "lucide-react";
...
export default function NfcChipsPage() {
  ...
  useEffect(() => { fetchChips(); fetchBatches(); fetch("/api/customers").then(r => r.json()).then(d => setCustomers(d.customers)); }, []);
  ...
  const handleGenerateBatch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.batchId) {
        // Immediately download the CSV of the new batch.
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="hidden md:block bg-card rounded-2xl border border-border overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-border"><th className="text-left px-5 py-3 text-xs font-medium text-muted uppercase tracking-wider">Chip UID</th><th className="text-left px-5 py-3 text-xs font-medium text-muted uppercase tracking-wider">Customer</th><th className="text-left px-5 py-3 text-xs font-medium text-muted uppercase tracking-wider">Status</th><th className="text-left px-5 py-3 text-xs font-medium text-muted uppercase tracking-wider">Registered</th><th className="text-left px-5 py-3 text-xs font-medium text-muted uppercase tracking-wider">Actions</th></tr></thead><tbody className="divide-y divide-border">{filtered.map(chip => <tr key={chip._id} className="hover:bg-background/50"><td className="px-5 py-3 font-mono text-xs">{chip.chipUid}</td><td className="px-5 py-3">{chip.customerName ? <span className="font-medium">{chip.customerName}</span> : <span className="text-muted italic">Unassigned</span>}</td><td className="px-5 py-3"><StatusBadge status={chip.status} /></td><td className="px-5 py-3 text-muted">{formatDate(chip.registeredAt)}</td><td className="px-5 py-3"><div className="relative"><button onClick={(e) => { e.stopPropagation(); setOpenMenu(openMenu === chip._id ? null : chip._id); }} className="p-1 rounded-lg hover:bg-sidebar-hover"><MoreHorizontal className="w-4 h-4 text-muted" /></button>{openMenu === chip._id && (<div onClick={(e) => e.stopPropagation()} className="absolute right-0 top-8 z-20 w-44 bg-card border border-border rounded-xl shadow-lg py-1">{chip.customerId && <button onClick={() => { setOpenMenu(null); router.push(`/customers/${chip.customerId}`); }} className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-left hover:bg-background transition-colors"><Eye className="w-4 h-4 text-muted" />View customer</button>}{chip.customerId ? <button onClick={() => { setOpenMenu(null); handleUnlink(chip._id); }} className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-left hover:bg-background transition-colors"><Unlink className="w-4 h-4 text-muted" />Unlink</button> : <button onClick={() => { setOpenMenu(null); setShowLink(chip._id); }} className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-left hover:bg-background transition-colors"><Link2 className="w-4 h-4 text-muted" />Link to customer</button>}<button onClick={() => toggleStatus(chip)} className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-left hover:bg-background transition-colors"><Power className="w-4 h-4 text-muted" />{chip.status === "active" ? "Disable" : "Activate"}</button><button onClick={() => { setOpenMenu(null); setConfirmDelete(chip); }} className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-left text-red-600 hover:bg-red-50 transition-colors"><Trash2 className="w-4 h-4" />Delete</button></div>)}</div></td></tr>)}</tbody></table></div></div>
      <div className="md:hidden space-y-3">{filtered.map(chip => <div key={chip._id} className="bg-card rounded-2xl border border-border p-4 space-y-3"><div className="flex items-center justify-between"><span className="font-mono text-xs">{chip.chipUid}</span><StatusBadge status={chip.status} /></div><div className="flex items-center justify-between"><div>{chip.customerName ? <p className="text-sm font-medium">{chip.customerName}</p> : <p className="text-sm text-muted italic">Unassigned</p>}<p className="text-xs text-muted mt-0.5">Registered {formatDate(chip.registeredAt)}</p></div><div className="flex items-center gap-2">{chip.customerId ? <button onClick={() => handleUnlink(chip._id)} className="flex items-center gap-1.5 px-3 py-2 bg-background rounded-xl text-xs font-medium text-muted hover:text-foreground transition-colors"><Unlink className="w-3 h-3" />Unlink</button> : <button onClick={() => setShowLink(chip._id)} className="flex items-center gap-1.5 px-3 py-2 bg-accent-light rounded-xl text-xs font-medium text-accent hover:opacity-80 transition-opacity"><Link2 className="w-3 h-3" />Link</button>}<button onClick={() => setConfirmDelete(chip)} className="flex items-center justify-center w-9 h-9 bg-red-50 rounded-xl text-red-600 hover:opacity-80 transition-opacity"><Trash2 className="w-3.5 h-3.5" /></button></div></div></div>)}</div>
