...
export default function NfcChipsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal open={showRegister} onClose={() => { setShowRegister(false); setNewUid(""); }} title="Register a Custom Code"><form onSubmit={handleRegister} className="space-y-4"><div><label className="block text-sm text-muted mb-1.5">Code</label><input type="text" value={newUid} onChange={e => setNewUid(e.target.value.toUpperCase())} placeholder="e.g. LT-JENNY" className="w-full px-4 py-2.5 bg-background border border-border rounded-xl text-sm font-mono focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent" required autoFocus /><p className="text-xs text-muted mt-1.5">Create a custom code (e.g. a vanity name) for a single chip. It becomes an unclaimed chip the worker can activate.</p><p className="text-xs text-warning mt-1.5">Note: custom codes are guessable — use random Generate Batch codes for sealed retail pendants.</p></div><div className="flex justify-end gap-2 pt-2"><button type="button" onClick={() => { setShowRegister(false); setNewUid(""); }} className="px-4 py-2.5 bg-background border border-border rounded-xl text-sm font-medium text-muted hover:text-foreground transition-colors">Cancel</button><button type="submit" className="px-4 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-medium hover:opacity-90 transition-opacity">Register</button></div></form></Modal>
