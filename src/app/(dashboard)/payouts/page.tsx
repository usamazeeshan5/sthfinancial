...
type Payout = { _id: string; customerName: string; amount: number; status: string; scheduledAt: string; completedAt: string | null };
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? <div className="space-y-3">{[1,2,3].map(i=><div key={i} className="h-16 bg-card rounded-2xl border border-border animate-pulse"/>)}</div> : <>
        <div className="hidden md:block bg-card rounded-2xl border border-border overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-border"><th className="text-left px-5 py-3 text-xs font-medium text-muted uppercase tracking-wider">Customer</th><th className="text-left px-5 py-3 text-xs font-medium text-muted uppercase tracking-wider">Amount</th><th className="text-left px-5 py-3 text-xs font-medium text-muted uppercase tracking-wider">Status</th><th className="text-left px-5 py-3 text-xs font-medium text-muted uppercase tracking-wider">Scheduled</th><th className="text-left px-5 py-3 text-xs font-medium text-muted uppercase tracking-wider">Completed</th><th className="text-left px-5 py-3 text-xs font-medium text-muted uppercase tracking-wider">Actions</th></tr></thead><tbody className="divide-y divide-border">{filtered.map(p => <tr key={p._id} className="hover:bg-background/50"><td className="px-5 py-3 font-medium">{p.customerName}</td><td className="px-5 py-3">{formatCurrency(p.amount)}</td><td className="px-5 py-3"><StatusBadge status={p.status} /></td><td className="px-5 py-3 text-muted">{formatDate(p.scheduledAt)}</td><td className="px-5 py-3 text-muted">{p.completedAt ? formatDateTime(p.completedAt) : "—"}</td><td className="px-5 py-3">{p.status === "failed" && <button onClick={() => handleRetry(p._id)} className="flex items-center gap-1.5 px-3 py-1.5 bg-danger-light text-danger rounded-lg text-xs font-medium hover:opacity-80 transition-opacity"><RotateCcw className="w-3 h-3" />Retry</button>}</td></tr>)}</tbody></table></div></div>
        <div className="md:hidden space-y-3">{filtered.map(p => <div key={p._id} className="bg-card rounded-2xl border border-border p-4 space-y-3"><div className="flex items-center justify-between"><span className="font-medium text-sm">{p.customerName}</span><StatusBadge status={p.status} /></div><p className="text-lg font-semibold">{formatCurrency(p.amount)}</p><div className="flex items-center justify-between text-xs text-muted"><span>Scheduled: {formatDate(p.scheduledAt)}</span><span>{p.completedAt ? formatDateTime(p.completedAt) : "Pending"}</span></div>{p.status === "failed" && <button onClick={() => handleRetry(p._id)} className="flex items-center gap-1.5 px-3 py-2 bg-danger-light text-danger rounded-xl text-xs font-medium hover:opacity-80 transition-opacity w-full justify-center"><RotateCcw className="w-3 h-3" />Retry Payout</button>}</div>)}</div>
      </>}
    </div>
  );
}
