...
export default function CustomersPage() {
  ...
  const [form, setForm] = useState({ name: "", email: "", phone: "", password: "", chipCode: "" });
  ...
  const handleAdd = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setForm({ name: "", email: "", phone: "", password: "", chipCode: "" });
    setShowAdd(false);
    setError("");
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal open={showAdd} onClose={() => { setShowAdd(false); setForm({ name: "", email: "", phone: "", password: "", chipCode: "" }); setError(""); }} title="Add New Customer">
        <form onSubmit={handleAdd} className="space-y-4">
          ...
          <div><label className="block text-sm text-muted mb-1.5">Chip Code <span className="text-muted/70">(optional)</span></label><input type="text" value={form.chipCode} onChange={(e) => setForm(f => ({ ...f, chipCode: e.target.value.toUpperCase() }))} placeholder="LT-XXXX-XXXX" className="w-full px-4 py-2.5 bg-background border border-border rounded-xl text-sm font-mono focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent" /><p className="text-xs text-muted mt-1">Assign an existing code from a batch. Leave blank to auto-generate a new one.</p></div>
          <div className="flex justify-end gap-2 pt-2"><button type="button" onClick={() => { setShowAdd(false); setForm({ name: "", email: "", phone: "", password: "", chipCode: "" }); setError(""); }} className="px-4 py-2.5 bg-background border border-border rounded-xl text-sm font-medium text-muted hover:text-foreground transition-colors">Cancel</button><button type="submit" className="px-4 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-medium hover:opacity-90 transition-opacity">Add Customer</button></div>
