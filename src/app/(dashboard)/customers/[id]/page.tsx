...
export default function CustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const changeCode = async () => {
    ...
    setChipBusy(true);
    const res = await fetch(`/api/customers/${id}/chip`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ chipCode: codeInput.trim() }),
    });
    const d = await res.json();
    setChipBusy(false);
    if (!res.ok) { setChipError(d.error || "Couldn't change the code."); return; }
    setChanging(false);
    setCodeInput("");
    load();
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-2xl border border-border">
        <div className="px-4 sm:px-5 py-4 border-b border-border flex items-center justify-between gap-2">
          <div className="flex items-center gap-2"><CreditCard className="w-4 h-4 text-muted" /><h2 className="text-sm font-medium">NFC Chips ({chips.length})</h2></div>
          {!changing && <button onClick={() => { setChanging(true); setChipError(""); }} className="text-sm font-medium text-accent hover:underline">Change code</button>}
        </div>
        <div className="p-4 sm:p-5 space-y-3">
          ...
          {changing && (
            <div className="pt-2 border-t border-border space-y-2">
              <label className="block text-sm text-muted">Assign a code from a batch</label>
              <div className="flex flex-col sm:flex-row gap-2">
                <input type="text" value={codeInput} onChange={(e) => setCodeInput(e.target.value.toUpperCase())} placeholder="LT-XXXX-XXXX" className="flex-1 px-4 py-2.5 bg-background border border-border rounded-xl text-sm font-mono focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent" autoFocus />
                <button onClick={changeCode} disabled={chipBusy || !codeInput.trim()} className="px-4 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-50">{chipBusy ? "Saving…" : "Save"}</button>
                <button onClick={() => { setChanging(false); setCodeInput(""); setChipError(""); }} className="px-4 py-2.5 bg-background border border-border rounded-xl text-sm font-medium text-muted hover:text-foreground transition-colors">Cancel</button>
              </div>
